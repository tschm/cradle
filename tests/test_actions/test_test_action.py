...
def test_test_action_structure(action_path):
    ...
    assert os.path.exists(test_action_path), (
        f"Action file not found at {test_action_path}"
    )
    ...
    assert inputs["tests-folder"]["default"] == "tests", (
        "Tests-folder input must default to tests"
    )
    ...
    run_tests_step = next(
        (step for step in steps if step.get("name", "").startswith("Run tests")), None
    )
    assert run_tests_step is not None, "Action must have a run tests step"

    # Check step details
    assert (
        run_tests_step["shell"] == "${{ runner.os == 'Windows' && 'pwsh' || 'bash' }}"
    ), "Run tests step must use cross-platform shell selection"
    assert run_tests_step["run"].find("uv pip install") != -1, (
        "Run tests step must install pytest"
    )
    assert run_tests_step["run"].find("uv run pytest") != -1, (
        "Run tests step must run pytest"
    )
    assert run_tests_step["run"].find("${{ inputs.tests-folder }}") != -1, (
        "Run tests step must use tests-folder input"
    )

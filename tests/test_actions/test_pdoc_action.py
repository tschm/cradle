...
def test_pdoc_action_structure(action_path):
    ...
    assert os.path.exists(pdoc_action_path), (
        f"Action file not found at {pdoc_action_path}"
    )
    ...
    assert inputs["source-folder"]["default"] == "cvx", (
        "Source-folder input must default to cvx"
    )
    assert inputs["pdoc-arguments"]["default"] == "", (
        "Pdoc-arguments input must default to empty string"
    )
    ...
    build_step = next(
        (
            step
            for step in steps
            if step.get("name", "").startswith("Install and build")
        ),
        None,
    )
    ...
    # Check step details
    assert build_step["run"].find("uv pip install pdoc") != -1, (
        "Build step must install pdoc"
    )
    assert build_step["run"].find("uv run pdoc") != -1, "Build step must run pdoc"
    assert build_step["run"].find("${{ inputs.pdoc-arguments }}") != -1, (
        "Build step must use pdoc-arguments input"
    )
    assert build_step["run"].find("${{ inputs.source-folder }}") != -1, (
        "Build step must use source-folder input"
    )

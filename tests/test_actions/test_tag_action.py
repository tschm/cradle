...
def test_tag_action_structure(action_path):
    ...
    assert os.path.exists(tag_action_path), (
        f"Action file not found at {tag_action_path}"
    )
    ...
    assert (
        outputs["new_tag"]["value"].find("steps.tag_version.outputs.new_tag") != -1

"""Utility to check the age of dependencies and generate reports."""
...
def check_dependency_ages():
    """Check the age of dependencies and generate reports."""
    try:
        ...
        with open("artifacts/age/dependency-age.txt", "w") as f:
            ...
            for dep in deps:
                ...
                if latest_version:
                    ...
                    package_data.append({"Package": pkg, "Latest Version": latest_version, "Last Update": release_date})

        # Create DataFrame with packages as rows (not columns)
        if package_data:
            dframe = pd.DataFrame(package_data)

            # Convert dates to datetime for proper sorting
            dframe["Last Update"] = pd.to_datetime(dframe["Last Update"])

            # Sort by date
            sorted_df = dframe.sort_values(by="Last Update", ascending=False)
